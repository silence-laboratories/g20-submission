    ...
    <PageContainer scrollable={false}>
      <div className='flex flex-1 flex-col space-y-4'>
        <div className='flex items-start justify-between'>
          <Heading
            title='Applications'
            description={user.entityType === "sme" ? 'Manage your Loan Applications' : 'Review loan applications from SMEs'}
          />
        </div>
        <Separator />
        <Suspense
        ...
        >
          {user.entityType === "sme" ? <SMEApplicationListingPage user={user}/> : <BankApplicationListingPage user={user}/>}
        </Suspense>
      </div>
    </PageContainer>
  );
