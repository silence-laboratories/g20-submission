...
export default function Register() {
  ...
  const [entityType, setEntityType] = useState<'sme' | 'bank' | 'individual' | null>(null)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!isVerified) {
      setErrors(prev => ({ ...prev, bankAccountNumber: 'Bank account number is not verified' }))
      return
    }
  ...
  const validateForm = (): boolean => {
    ...
    if (entityType === 'sme') {
      ...
      if (!(formData.phoneNumber?.trim().length === 10)) newErrors.phoneNumber = 'Phone number must be 10 digits'
      ...
      if (!(formData.bankAccountNumber?.trim().length === 10)) newErrors.bankAccountNumber = 'Bank account number must be 10 digits'
    }
    setErrors(newErrors)
  ...
  const handleVerifyAccount = async () => {
    if (!formData.bankAccountNumber?.trim()) {
      ...
      setErrors(prev => ({ ...prev, bankAccountNumber: 'Bank account number is required for verification' }))
      return
    }

    if (!(formData.bankAccountNumber?.trim().length === 10)) {
      setFormValidated(false)
      setErrors(prev => ({ ...prev, bankAccountNumber: 'Bank account number must be 10 digits' }))
      return
    }
                ...
                <CardContent>
                  <div className="grid md:grid-cols-2 gap-6">
                    <div>
                      ...
                      <div className="py-2 rounded-lg text-white font-bold">
                  ...
                  <div className="mt-4 p-4 border border-green-200 dark:bg-green-950/20 dark:border-green-800 rounded-lg">
                    <div className='flex items-center'>
                      <div className="flex-shrink-0 w-8 h-8 rounded-lg bg-green-100 dark:bg-green-900/50 flex items-center justify-center mr-4">
                        <IconInfoCircle className="w-4 h-4 text-green-600 dark:text-green-400" />
                      </div>
                      <p className="text-sm text-green-600">
                        These details are automatically filled from your Google account and cannot be changed here.
                      </p>
                    </div>
                  </div>
                </CardContent>
              ...
              <Card className='mx-auto w-full mb-4'>
                ...
                <CardContent>
                  {!entityType && (
                    <div className="grid md:grid-cols-2 gap-6">
                      <div
                        className="border border-gray-500 rounded-xl p-6 cursor-pointer hover:border-blue-500 hover:shadow-lg transition-all"
                        onClick={() => {
                          setEntityType('sme')
                        }}
                      >
                        <div className="flex items-center mb-4">
                          ...
                          <h3 className="text-xl font-semibold text-white">SME (Small & Medium Enterprise)</h3>
                        </div>
                        <p className="text-gray-200">
                          Register as a small or medium enterprise to apply for loans and manage your business financing.
                        </p>
                      </div>

                      <div className="relative">
                        <div className="absolute bottom-0 left-0 right-0 bg-orange-300 text-black text-center py-2 px-2 rounded-b-xl font-medium text-sm z-10">
                          Coming Soon
                        </div>
                        <div
                          className="border border-gray-500 rounded-xl p-6 opacity-50 cursor-not-allowed transition-all relative"
                        >
                          <div className="flex items-center mb-4">
                            <div className="w-12 h-12 bg-primary/10 rounded-lg flex items-center justify-center mr-4">
                              <svg className="w-6 h-6 text-primary" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                          ...
                          <p className="text-gray-200 mb-4">
                            Register as an individual to apply for loans and manage your personal financing.
                          </p>
                        </div>
                      </div>
                    </div>
                    ...
                    <>
                      <div className="flex items-center justify-between mb-6">
                        <h2 className="text-2xl font-bold text-white">
                          {entityType === 'sme' ? 'SME Registration' : 'Bank Registration'}
                        </h2>
                        <Button variant="ghost" onClick={() => setEntityType(null)} className='cursor-pointer'>
                          Change Entity
                        </Button>
                      </div>
                      ...
                      <form onSubmit={handleSubmit} className="space-y-6 w-full">
                        ...
                        <div>
                          {formData.country && (
                            <Card>
                              <CardHeader>
                                <CardTitle className="text-xl">Select your Primary Banking Partner</CardTitle>
                                <p className="text-muted-foreground">Choose the bank where you maintain your primary business account in {formData.country}</p>
                              </CardHeader>
                              <CardContent>
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                  {banks.length > 0 ? banks?.map((bank) => (
                                    ...
                                    <div className="flex items-center space-x-2">
                                      ...
                                      <span>Fetching banks for {formData.country}...</span>
                                    </div>
                                  )}
                                </div>
                                    ...
                                    <div className="flex gap-2">
                                      ...
                                      {!isVerified ? (
                                        <Button
                                          ...
                                          disabled={isVerifying || !formData.bankAccountNumber?.trim()}
                                          className={cn(
                                        ...
                                        <div className="flex items-center gap-2 h-9 px-4 bg-green-100 dark:bg-green-900/50 text-green-700 dark:text-green-400 rounded-md border border-green-200 dark:border-green-800">
                        ...
                        <div className="flex space-x-4 pt-6">
                          <Button type="submit" disabled={isSubmitting || !formValidated} className='cursor-pointer'>
                            {isSubmitting ? (
                          ...
                          <Button variant="secondary" onClick={() => setEntityType(null)} className='cursor-pointer'>
                            Cancel
                          </Button>
                        </div>
                      </form>

                      <MapDemo getSelectedCountry={countrySelect} title='Country of Incorporation *'/>
                      </div>
                    </>
                  )}
                </CardContent>
