...
import { Shield, Lock, CheckCircle, Package, Send, ArrowRight, Clock, Zap, Search, MapPin } from 'lucide-react';
import { geoCylindricalStereographic } from 'd3-geo-projection';
import { Tooltip } from "react-tooltip";
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from "@/components/ui/select";

// Coordinates for Mumbai, India and London, UK
const ORIGIN = { name: 'Mumbai', coords: [72.8777, 19.0760] };
...
const COUNTRIES = [
    ...
    { id: "CAF", name: "Central African Republic", coords: [20.939444, 6.611111] },
    ...
    { id: "COD", name: "Democratic Republic of Congo", coords: [21.758664, -4.038333] },
...
export default function SecureTransferBoard() {
    ...
    return (
        <div className='w-full'>
                ...
                {selectedCountry && (
                    <div className="flex items-center gap-2">
                        <div className="text-sm text-gray-300">
                            Viewing: <span className="font-semibold text-blue-400">
            ...
            <div className="relative w-full h-[600px] bg-[#10172A] rounded-xl overflow-hidden shadow-lg p-4 mb-8">
                <ComposableMap
                ...
                >
                    ...
                    <Geographies geography="/features.json">
                        {({ geographies }) =>
                            geographies.map(geo => {
                                const isSelected = selectedCountry && geo.id === selectedCountry;
                                return (
                    ...
                    <Marker coordinates={ORIGIN.coords as [number, number]}>
                        ...
                        />
                        <g className="absolute top-20 left-1/2 transform -translate-x-1/2 whitespace-nowrap">
                            <g className="bg-slate-800 px-3 py-1 rounded-lg border border-blue-400 shadow-lg">
                ...
                <div className="absolute bottom-2 right-2 flex gap-2">
                    <button onClick={() => setScene(Math.max(scene - 1, 0))}
                        className="rounded-full px-3 py-1 bg-gray-500 text-white">
                        Prev
                    </button>
                    <button onClick={() => setScene(Math.min(scene + 1, SCENES.length - 1))}
                        className="rounded-full px-3 py-1 bg-blue-600 text-white">
                        Next
                    </button>
                </div>
            </div>
...
function SceneOverlay({ scene }: { scene: any }) {
    switch (scene) {
        case 'setup':
            return (
                <div className="absolute bottom-4 left-1/2 -translate-x-1/2 bg-[#16213e] px-8 py-3 rounded-lg text-white shadow-lg flex items-center gap-3">
                    <span className="text-xl font-semibold">Preparing Secure Data Transfer</span>
                </div>
            );
        case 'collect':
            return (
                <motion.div
                    key="collect"
                    className="absolute top-4 right-8 bg-[#1B2535] p-5 rounded-lg shadow-xl text-white w-[320px]"
                    initial={{ opacity: 0, x: 60 }}
                    animate={{ opacity: 1, x: 0 }}
                    exit={{ opacity: 0, x: 40 }}
                >
                    <div className="font-bold mb-3">Collecting approved data...</div>
        ...
        case 'encrypt':
            return (
                <motion.div
                    key="encrypt"
                    className="absolute left-1/2 bottom-9 -translate-x-1/2 bg-[#11204d] p-5 rounded-lg border-4 border-blue-500 text-white shadow-2xl min-w-[320px]"
                    initial={{ opacity: 0, y: 40 }}
                    animate={{ opacity: 1, y: 0 }}
                    exit={{ opacity: 0, y: 20 }}
                >
                    <div>
                        <span className="font-bold text-cyan-400">🔒 2048-bit RSA + AES-256</span>
                    </div>
                    <div className="mb-2">Data is now unreadable to unauthorized parties.</div>
        ...
        case 'tunnel':
            return (
                <motion.div
                    key="tunnel"
                    className="absolute left-1/2 bottom-8 -translate-x-1/2 rounded-lg px-7 py-3 bg-[#122620] text-white border-2 border-cyan-300 shadow-2xl"
                ...
                >
                    <div>
                        ...
                        <div className="text-sm text-cyan-200">Quantum-resistant encryption enabled</div>
                    </div>
                </motion.div>
            );
        default:
...
const SecureTransferJourney = () => {
    ...
    return (
        <div className="min-h-screen bg-gradient-to-br from-slate-900 via-blue-900 to-slate-900 text-white p-8 overflow-hidden">
            {/* Header */}
            <div className="max-w-7xl mx-auto mb-8">
                <div className="flex items-center justify-between">
                    <div>
                        <h1 className="text-3xl font-bold mb-2">{getStageTitle()}</h1>
            ...
            <div className="max-w-7xl mx-auto relative">
                ...
                <div className="relative h-96 bg-slate-800/50 rounded-2xl border border-blue-500/30 backdrop-blur-sm overflow-hidden">
                    {/* Grid Pattern */}
                    ...
                    {/* Origin Point - Mumbai */}
                    <div className={`absolute left-1/4 top-1/2 transform -translate-x-1/2 -translate-y-1/2 transition-all duration-1000 ${stage >= 0 ? 'opacity-100 scale-100' : 'opacity-0 scale-0'}`}>
                        <div className="relative">
                            {/* Pin */}
                            <div className={`w-16 h-16 rounded-full bg-gradient-to-br from-blue-400 to-blue-600 border-4 border-white shadow-2xl flex items-center justify-center ${stage >= 1 ? 'animate-pulse' : ''}`}>
                                <Package className="w-8 h-8" />
                            </div>
                            {/* Label */}
                            <div className="absolute top-20 left-1/2 transform -translate-x-1/2 whitespace-nowrap">
                                <div className="bg-slate-800 px-3 py-1 rounded-lg border border-blue-400 shadow-lg">
                            ...
                            {stage === 1 && (
                                <>
                                    {[...Array(4)].map((_, i) => (
                                        ...
                                        >
                                            <div className="w-8 h-8 bg-blue-400 rounded opacity-75"></div>
                                        </div>
                                    ))}
                                </>
                            )}
                            ...
                            {stage === 2 && (
                                <>
                                    {encryptionLayers >= 1 && (
                                        <div className="absolute inset-0 -m-2 rounded-full border-2 border-cyan-400 animate-pulse"></div>
                                    )}
                                    {encryptionLayers >= 2 && (
                                        <div className="absolute inset-0 -m-4 rounded-full border-2 border-cyan-300 animate-pulse" style={{ animationDelay: '0.3s' }}></div>
                                    )}
                                    {encryptionLayers >= 3 && (
                                        <div className="absolute inset-0 -m-6 rounded-full border-4 border-blue-500 animate-pulse shadow-2xl shadow-blue-500/50" style={{ animationDelay: '0.6s' }}></div>
                                    )}
                                </>
                            )}
                        </div>
                    ...
                    <div className={`absolute right-1/4 top-1/2 transform translate-x-1/2 -translate-y-1/2 transition-all duration-1000 ${stage >= 0 ? 'opacity-100 scale-100' : 'opacity-0 scale-0'}`}>
                        <div className="relative">
                            {/* Pin */}
                            <div className={`w-16 h-16 rounded-full bg-gradient-to-br from-teal-400 to-green-600 border-4 border-white shadow-2xl flex items-center justify-center ${stage >= 6 ? 'animate-pulse' : ''}`}>
                                <Shield className="w-8 h-8" />
                            </div>
                            {/* Label */}
                            <div className="absolute top-20 left-1/2 transform -translate-x-1/2 whitespace-nowrap">
                                <div className="bg-slate-800 px-3 py-1 rounded-lg border border-green-400 shadow-lg">
                            ...
                            {stage === 6 && (
                                <div className="absolute inset-0 -m-8">
                                    <div className="w-full h-full rounded-full border-4 border-green-400 animate-ping"></div>
                                </div>
                            )}

                            {/* Success Glow */}
                            {stage >= 8 && (
                                <div className="absolute inset-0 -m-4 rounded-full bg-green-400/20 animate-pulse"></div>
                            )}
                        </div>
                    </div>
                    ...
                    {/* Secure Tunnel */}
                    {stage >= 3 && (
                        <div className="absolute left-1/4 right-1/4 top-1/2 transform -translate-y-1/2">
                            <svg className="w-full h-24" viewBox="0 0 100 24" preserveAspectRatio="none">
                                {/* Tunnel Base */}
                                <defs>
                            ...
                            {stage >= 3 && (
                                <>
                                    <div className="absolute left-1/3 top-1/2 transform -translate-x-1/2 -translate-y-1/2">
                                        <div className={`w-4 h-4 rounded-full bg-cyan-400 shadow-lg shadow-cyan-400/50 ${stage >= 5 ? 'animate-pulse' : ''}`}></div>
                                    </div>
                            ...
                            {stage === 4 && (
                                <>
                                    <div className="absolute left-1/4 top-1/2 transform -translate-y-1/2 animate-pulse">
                                        <Lock className="w-6 h-6 text-yellow-400" />
                                    </div>
                                    <div className="absolute right-1/4 top-1/2 transform -translate-y-1/2 animate-pulse">
                                        <Lock className="w-6 h-6 text-yellow-400" />
                                    </div>
                                </>
                            )}
                            ...
                            {stage === 5 && (
                                ...
                                >
                                    <div className="relative">
                                        <div className="w-10 h-10 bg-gradient-to-br from-cyan-400 to-blue-600 rounded-lg shadow-2xl shadow-cyan-400/50 flex items-center justify-center animate-pulse">
                                            <Send className="w-5 h-5" />
                                        </div>
                                        {/* Trail effect */}
                                        <div className="absolute inset-0 bg-cyan-400 rounded-lg blur-xl opacity-50 animate-pulse"></div>
                                    </div>
                                </div>
                            )}
                        </div>
                    ...
                    {stage >= 8 && (
                        <div className="absolute top-1/4 left-1/2 transform -translate-x-1/2 -translate-y-1/2">
                            <div className="w-20 h-20 bg-green-500 rounded-full flex items-center justify-center shadow-2xl shadow-green-500/50 animate-bounce">
                ...
                {stage === 5 && (
                    <div className="mt-6 bg-slate-800/50 rounded-xl p-4 border border-blue-500/30">
                        <div className="flex justify-between items-center mb-2">
                            ...
                            <span className="text-sm text-blue-300">{progress.toFixed(0)}%</span>
                        </div>
                        <div className="w-full bg-slate-700 rounded-full h-3 overflow-hidden">
                            <div
                                className="h-full bg-gradient-to-r from-cyan-400 to-blue-600 transition-all duration-200 rounded-full shadow-lg shadow-cyan-400/50"
                ...
                <div className="mt-6 grid grid-cols-1 md:grid-cols-3 gap-4">
                    {/* Security Status */}
                    <div className="bg-slate-800/50 rounded-xl p-4 border border-blue-500/30 backdrop-blur-sm">
                        ...
                        <div className="space-y-2 text-sm">
                            <div className="flex items-center justify-between">
                                ...
                                <span className={`font-semibold ${stage >= 2 ? 'text-green-400' : 'text-gray-400'}`}>
                                    {stage >= 2 ? 'AES-256' : 'Pending'}
                                </span>
                            </div>
                            <div className="flex items-center justify-between">
                                <span className="text-blue-200">Channel</span>
                                <span className={`font-semibold ${stage >= 3 ? 'text-green-400' : 'text-gray-400'}`}>
                                    {stage >= 3 ? 'TLS 1.3' : 'Pending'}
                                </span>
                            </div>
                            <div className="flex items-center justify-between">
                                <span className="text-blue-200">Verification</span>
                                <span className={`font-semibold ${stage >= 4 ? 'text-green-400' : 'text-gray-400'}`}>
                    ...
                    <div className="bg-slate-800/50 rounded-xl p-4 border border-blue-500/30 backdrop-blur-sm">
                        ...
                        <div className="space-y-2 text-sm">
                            {['Collected', 'Encrypted', 'Secured', 'Transferred', 'Verified'].map((label, i) => (
                                <div key={i} className="flex items-center gap-2">
                                    ...
                                    <span className={stage > i + 1 ? 'text-green-400' : stage === i + 1 ? 'text-blue-400' : 'text-gray-400'}>
                                        {label}
                                    </span>
                                </div>
                            ))}
                    ...
                    <div className="bg-slate-800/50 rounded-xl p-4 border border-blue-500/30 backdrop-blur-sm">
                        ...
                        <div className="space-y-2 text-sm">
                            <div className={`flex items-center gap-2 transition-all ${encryptionLayers >= 1 ? 'text-cyan-400' : 'text-gray-500'}`}>
                                <div className={`w-3 h-3 rounded-full ${encryptionLayers >= 1 ? 'bg-cyan-400' : 'bg-gray-600'}`}></div>
                                <span>AES-256 Applied</span>
                            </div>
                            <div className={`flex items-center gap-2 transition-all ${encryptionLayers >= 2 ? 'text-cyan-400' : 'text-gray-500'}`}>
                                <div className={`w-3 h-3 rounded-full ${encryptionLayers >= 2 ? 'bg-cyan-400' : 'bg-gray-600'}`}></div>
                                <span>End-to-End Active</span>
                            </div>
                            <div className={`flex items-center gap-2 transition-all ${encryptionLayers >= 3 ? 'text-blue-400' : 'text-gray-500'}`}>
                                <div className={`w-3 h-3 rounded-full ${encryptionLayers >= 3 ? 'bg-blue-400' : 'bg-gray-600'}`}></div>
                ...
                {showDetails && stage >= 2 && (
                    <div className="mt-6 bg-slate-800/70 rounded-xl p-6 border border-blue-500/30 backdrop-blur-sm animate-in fade-in duration-500">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-sm">
                            <div>
                                <p className="text-blue-300 mb-2">Encryption Standards</p>
                            ...
                            <div>
                                <p className="text-blue-300 mb-2">Transfer Protocol</p>
                                <ul className="space-y-1 text-gray-300">
                            ...
                            <div>
                                <p className="text-blue-300 mb-2">Compliance</p>
                                <ul className="space-y-1 text-gray-300">
                            ...
                            <div>
                                <p className="text-blue-300 mb-2">Audit Trail</p>
                                <ul className="space-y-1 text-gray-300">
                ...
                {/* Success Summary */}
                {stage >= 8 && (
                    <div className="mt-6 bg-gradient-to-r from-green-900/50 to-emerald-900/50 rounded-xl p-6 border border-green-500/30 backdrop-blur-sm animate-in fade-in duration-500">
                        <div className="flex items-start gap-4">
                            ...
                            <div className="flex-1">
                                ...
                                <p className="text-green-100 mb-4">
                                    Your data has been securely transferred from Mumbai to London using military-grade encryption.
                                </p>
                                ...
                                <div className="mt-4 flex gap-3">
                                    <button className="px-4 py-2 bg-green-600 hover:bg-green-700 rounded-lg transition-colors text-sm font-semibold">
                                        Download Certificate
                                    </button>
                                    <button className="px-4 py-2 bg-slate-700 hover:bg-slate-600 rounded-lg transition-colors text-sm font-semibold">
                                        View Security Log
                                    </button>
                                    <button
                                        onClick={() => {
