...
const NewMap = forwardRef<NewMapRef, NewMapProps>(({ chartID, getSelectedCountry }, ref) => {
  ...
  const initMap = (): { polygons: any, points: any } => {
    ...
    if (rootRef.current) {
      ...
      let chart = rootRef.current.container.children.push(am5map.MapChart.new(rootRef.current, {
        panX: "none",
        panY: "none",
      ...
      let polygonSeries = chart.series.push(am5map.MapPolygonSeries.new(rootRef.current, {
      ...
      let pointSeries = chart.series.push(am5map.MapPointSeries.new(rootRef.current, {}));
  ...
  const addPoints = (pointSeries: any, data: any) => {
    ...
    pointSeries.data.setAll(data.map((item: any) => ({
  ...
  return (
    <div className="flex flex-col gap-4 w-full">
      <CountrySearch onCountrySelect={(countryCode, countryName) => {
        if (polygonSeries) {
          const dataItem = polygonSeries.dataItems.find((item: any) =>
            item.dataContext?.id === countryCode
          );
          if (dataItem) {
      ...
      <Card><CardContent><div id={chartID} style={{ height: "500px", width: "100%" }}></div></CardContent></Card>
    </div>
  );
});
