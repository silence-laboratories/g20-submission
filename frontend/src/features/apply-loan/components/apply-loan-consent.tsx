        ...
        if (user && user.entityId) {
            ...
            const getBanks = async (entityId: number) => {
                try {
                    ...
                    setBankAccounts([{ id: bank.id, bankName: bank.name, accountNumber: sme.bank_account_number, icon: IconBuildingBank }]);
                    setLoan(getLoans()[0]);
                }
                catch (e) {
                    console.log(e);
    ...
    return (
        <div className="max-w-5xl p-6 space-y-8">
            ...
            <div className="space-y-2">
                <h1 className="text-2xl font-bold text-foreground">Consent Request</h1>
                <p className="text-sm text-muted-foreground">Review and approve data sharing permissions.</p>
            </div>
                ...
                <CardContent>
                    <div>
                        <div className='flex items-center mb-4'>
                            ...
                            <h3 className="font-semibold text-green-900 dark:text-green-100 ml-2">Data Security & Privacy
                            </h3>
                        </div>
                            ...
                            <div className="space-y-3">
                                {securityPoints.map((point, index) => (
                                    <div key={index} className="flex items-start space-x-3">
                                        ...
                                        <p className="text-sm text-green-800 dark:text-green-200">{point}</p>
                                    </div>
                                ))}
                            </div>
                        </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <Card className="bg-muted/30">
                    <CardHeader>
                        <CardTitle className="text-xl font-semibold">Consent Terms</CardTitle>
                    </CardHeader>
                    <CardContent className="space-y-4">
                        <div className="space-y-2">
                            <h4 className="font-medium text-foreground">Purpose of Fetch:</h4>
                            <p className="text-sm text-muted-foreground">{loan?.type}</p>
                        </div>

                        {/* One year from today */}
                        <div className="space-y-2">
                            <h4 className="font-medium text-foreground">Consent Expiry:</h4>
                            <p className="text-sm text-muted-foreground">{new Date(Date.now() + 365 * 24 * 60 * 60 * 1000).toLocaleDateString()}</p>
                        </div>

                        <div className="space-y-2">
                            <h4 className="font-medium text-foreground">Validity of Consent:</h4>
                            <p className="text-sm text-muted-foreground">Valid until {new Date(Date.now() + 365 * 24 * 60 * 60 * 1000).toLocaleDateString()} or consent withdrawal</p>
                        </div>

                        <div className="space-y-2">
                            <h4 className="font-medium text-foreground">Frequency of Fetch:</h4>
                            <p className="text-sm text-muted-foreground">One-time during application process, periodic updates if loan approved</p>
                        </div>
                    </CardContent>
                </Card>
                ...
                <Card className="bg-muted/30">
                    ...
                    <CardContent className="space-y-4">
                        ...
                        <div className="space-y-2">
                            <h4 className="font-medium text-foreground">Selected Country of Institution:</h4>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <Card className="bg-muted/30">
                    <CardHeader>
                        <CardTitle className="text-xl font-semibold">Inferences Request</CardTitle>
                    </CardHeader>
                    <CardContent>
                        ...
                        <div className="grid grid-cols-1 gap-4">
                            {inferenceRequests.map((request) => {
                                ...
                                return (
                                    <div
                                        key={request.id}
                                        className="flex items-start space-x-3 p-4 rounded-lg bg-background border border-border"
                                    >
                ...
                <Card className="bg-muted/30">
                    <CardHeader>
                        <div className="space-y-2">
                            <CardTitle className="text-xl font-semibold">Select Accounts</CardTitle>
                            <Card className="bg-green-50 border-green-200 dark:bg-green-950/20 dark:border-green-800 mb-4 mt-4">
                                <CardContent>
                                    <div>
                                        <div className="flex-1">
                                            <div className="space-y-3">
                                                <div className="flex items-start space-x-3">
                                                    ...
                                                    <p className="text-sm text-green-800 dark:text-green-200">Requested insights will be generated from financial data soruces like Banks, Tax Authorities, Credit Bureaus etc</p>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                            ...
                            <p className="text-sm text-muted-foreground">Choose which accounts to share insights from</p>
                        </div>
                    </CardHeader>
                    <CardContent>
                        <div className="space-y-3">
                            <div className="flex items-center justify-between">
                                <h4 className="font-medium text-foreground">Available Accounts</h4>
                                ...
                                {bankAccounts.length > 0 ? bankAccounts.map((account) => {
                                    ...
                                    return (
                                        <div
                                            ...
                                            onClick={() => loan?.consent_status === "pending" && handleAccountToggle(account.id)}
                                        >
                                            <div className="flex-shrink-0 w-8 h-8 rounded-md bg-primary/10 flex items-center justify-center">
                                                <IconComponent className="w-4 h-4 text-primary" />
                                            </div>
                                            <div className="flex-1">
                                                <div className="flex items-center justify-between">
                                                    <div>
                                                        ...
                                                        <p className="text-sm text-muted-foreground">Account ending in {account.accountNumber.slice(-4)}.</p>
                                                    </div>
                            ...
                            {loan?.consent_status === "approved" ?
                                ...
                                </div> : loan?.consent_status === "rejected" ? <div className="flex items-start space-x-3">
                                    <div className="flex-shrink-0 w-5 h-5 rounded-full bg-red-200 dark:bg-red-800 flex items-center justify-center mt-0.5">
                                        <IconX className="w-3 h-3 text-white" />
                                    </div>
                                    <p className="text-sm text-red-800 dark:text-red-200">Consent rejected</p>
                                </div> : null}
                        </div>
                    </CardContent>
                </Card>
            ...
            <div className="flex justify-center space-x-4 pt-6">
                <Button className='cursor-pointer mr-4' variant="outline" onClick={onReject}>
                    Back
                </Button>

                {loan?.consent_status === "pending" ?
                    <>
                        <ConsentRejectDialog onReject={onReject || (() => { })} disabled={false} loanId={loan?.id} />
                        {bankAccounts.length > 0 &&
                            <ConsentOTPDialog onSubmit={onOTPSubmit || (() => { })} disabled={selectedAccountIds.length === 0} bankName={bankAccounts[0].bankName} loanId={loan?.id} />}
                    </> : <Button className='cursor-pointer mr-4' variant="default" onClick={onOTPSubmit}>
                        Next step
                    </Button>}
            </div>
        </div>
